...
class UserController extends AdminbaseController{
    ...
    public function disableUser() {
    	...
    	$res = $wxUserModel->where(['openid'=>$openid])->setField('status',0);
    	if (empty($res)) {
    		$this -> ajaxReturn(['result'=>false,'msg' => '禁用失败']);
    	}
    	$this -> ajaxReturn(['result'=>true,'msg' => '已禁用']);
    }
    ...
    public function enableUser() {
    	...
    	$wxUserModel = M('wx_user');
    	$res = $wxUserModel->where(['openid'=>$openid])->setField('status',1);
    	if (empty($res)) {
    		$this -> ajaxReturn(['result'=>false,'msg' => '解禁失败']);
    	}
    	$this -> ajaxReturn(['result'=>true,'msg' => '已解禁']);
    }
